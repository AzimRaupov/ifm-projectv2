...
class GenerateRodmap
{
    public static function generateRodmap($req, $user)
    {
        ...
        $prompt = "Создай пошаговый план для человека, которому ".$user->old." лет, и который может уделить ".$req->freetime." часов в неделю на изучение. Тема для изучения: '".$req->topic."'.

    Требование к курсу:
    topic_course={$req->topic} можеш только испровлять отпечатки
    ...
    - Каждый шаг должен содержать:
        ...
        2. Количество опыта, получаемого за выполнение этого шага (общий опыт должен суммироваться до 100).
